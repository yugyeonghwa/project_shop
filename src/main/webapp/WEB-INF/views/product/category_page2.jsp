		...
		<div class="cartalog_page">
				...
				<div class="model_best_background">
					...
					<div class="model_best_wrapper_item">
						...
						<c:forEach var="k" items="${pro_list}">
							<a href="product_detail_page.do"><img
								src="./resources/images/img01.jpg" class="slideshow-img"
								quality="high"></a>
						</c:forEach>
					</div>
					...
					<div class="model_best_wrapper_item2">
						<c:forEach var="k" items="${pro_list}">
							<span class="model_small_box"><img
								src="./resources/images/img01.jpg" class="small_box_img"></span>
						</c:forEach>
					</div>
				</div>
			</div>
			...
			<div class="model_box">
				<div class="model_box_head">
					...
					<span class="item_number" style="cursor: pointer;">${pro_list.size()}
						item</span> <input class="select_button" type="button" value="가격높은순"
						style="cursor: pointer;" oninput="" /> <input
						class="select_button" type="button" value="가격낮은순"
						style="cursor: pointer;" oninput="" />
				</div>
					...
					<div class="thumbs_wrapper">
						...
						<c:forEach var="k" items="${pro_list}">
							<div class="thumbs_item">
								<a href="product_detail_page.do"><img
									src="./resources/images/img01.jpg" class="thumbs_img" /></a>
								<div class="thumbs_info">
									<div class="thumbs_text">
										<span class="thumbs_title">${k.p_name}</span> <span
											class="thumbs_owner">${k.p_price}</span>
									</div>
									<div class="thumbs_emo_box">
										<c:choose>
											<c:when test="${k.active == 0}">
												<span class="wish_btn"><button class="wishlist-btn" type="button" value="${k.p_idx}">찜하기</button></span>
											</c:when>
											<c:otherwise>
												<span class="wish_btn"><button class="wishlist-btn" type="button" value="${k.p_idx}">찜하기완료</button></span>
											</c:otherwise>
										</c:choose>
									</div>
								</div>
