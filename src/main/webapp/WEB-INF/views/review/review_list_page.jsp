...
<html>
	<head>
		...
		<script type="text/javascript">
			function review_write_go() {
				location.href="review_write_go.do";
			}
			
            $(document).ready(function() {
                const reviewOwner = "${reviewOwner}";
                if (reviewOwner == 'no') {
                    alert("작성자만 볼 수 있습니다.");
                    return;
                }
	...
	<body>
		...
		<section>
										...
										<tr>
											...
											<td style="text-align: left;">
												...
												<c:choose>
													...
													<c:otherwise>
														<a href="review_detail.do?review_idx=${k.review_idx}&cPage=${paging.nowPage}">${k.review_subject}</a>
													</c:otherwise>
												</c:choose>
											</td>
											<td>${k.u_idx}</td>
								...
								<td colspan="5">
									<ol class="btn_paging">
										<c:choose>
											...
											<c:otherwise>
												<li>
													<a href="review_list.do?cPage=${paging.beginBlock - paging.pagePerBlock}">&#8249;</a>
												</li>
											</c:otherwise>
										</c:choose>
			...
			<input type="button" onclick="review_write_go()" value="글쓰기">
			</div>
